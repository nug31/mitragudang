...
// ============================================
// Stock In/Out Management Endpoints
// ============================================

// Initialize stock tracking table if it doesn't exist
async function ensureStockTrackingTable() {
  try {
    const [tables] = await pool.query("SHOW TABLES LIKE 'stock_tracking'");
    if (tables.length === 0) {
      console.log('Creating stock_tracking table...');
      await pool.query(`
      ...
      `);
      console.log('✅ stock_tracking table created');
    }
  } catch (error) {
    console.error('Error initializing stock tracking table:', error);
  }
}

// Call on startup
if (pool) {
  ensureStockTrackingTable().catch(err => console.error('Stock table init error:', err));
}

// Get all available items for selection (admin stock in/out)
app.get("/api/stock/available-items", async (req, res) => {
  try {
    console.log("GET /api/stock/available-items");
...
app.get("/api/stock/item/:id", async (req, res) => {
  try {
    const { id } = req.params;
    console.log(`GET /api/stock/item/${id}`);
...
app.post("/api/stock/in", async (req, res) => {
  try {
    const { itemId, quantity, notes, userId, unit } = req.body;
    
    console.log(`POST /api/stock/in - Item: ${itemId}, Qty: ${quantity}`);
    ...
    // Record in stock tracking
    const [result] = await pool.query(
      `INSERT INTO stock_tracking (item_id, item_name, type, quantity, unit, notes, created_by, created_at)
       VALUES (?, ?, 'in', ?, ?, ?, ?, NOW())`,
      [itemId, item.name, quantity, unit || item.unit || 'pcs', notes || '', userId || null]
    );

    console.log(`✅ Stock in recorded: Item ${itemId}, +${quantity} units`);
    ...
    res.status(500).json({
      success: false,
      message: "Error recording stock in",
      error: error.message
    });
  }
});
...
app.post("/api/stock/out", async (req, res) => {
  try {
    ...
    console.log(`POST /api/stock/out - Item: ${itemId}, Qty: ${quantity}`);
    ...
    console.log(`✅ Stock out recorded: Item ${itemId}, -${quantity} units`);
    ...
    res.status(500).json({
      success: false,
      message: "Error recording stock out",
      error: error.message
    });
  }
});
...
app.get("/api/stock/history", async (req, res) => {
  try {
    ...
    let query = "SELECT * FROM stock_tracking WHERE 1=1";
    const params = [];
